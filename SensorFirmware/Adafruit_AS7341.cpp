...
bool Adafruit_AS7341::readResults(uint16_t* readings_buffer) {
...
void Adafruit_AS7341::setSMUXCustomChannels(uint8_t adc_config[]) {
  enableSpectralMeasurement(false);
  setSMUXCommand(AS7341_SMUX_CMD_WRITE);
  setup_custom(adc_config);
  enableSMUX();
}
...
/**
 * @brief Configure SMUX for custom sensor setup
 *
 */
void Adafruit_AS7341::setup_custom(uint8_t adc_setting[]) {
  // F1,F2,F3,F4,F5,F6,F7,F8,CLEAR,NIR
  writeRegister(byte(0x00), adc_setting[2]<<4);     // F3 left
  writeRegister(byte(0x01), adc_setting[0]);        // F1 left
  writeRegister(byte(0x02), byte(0x00));            // Reserved or disabled
  writeRegister(byte(0x03), adc_setting[7]<<4);     // F8 left
  writeRegister(byte(0x04), adc_setting[5]);        // F6 left
  writeRegister(byte(0x05), adc_setting[3]<<4 | adc_setting[1]);   // F4 left | F2 left
  writeRegister(byte(0x06), adc_setting[4]<<4);     // F5 left
  writeRegister(byte(0x07), adc_setting[6]);        // F7 left 
  writeRegister(byte(0x08), adc_setting[8]<<4);     // CLEAR left
  writeRegister(byte(0x09), adc_setting[4]<<4);     // F5 right
  writeRegister(byte(0x0A), adc_setting[6]);        // F7 right
  writeRegister(byte(0x0B), byte(0x00));            // Reserved or disabled
  writeRegister(byte(0x0C), adc_setting[1]<<4);     // F2 right
  writeRegister(byte(0x0D), adc_setting[3]);        // F4 right
  writeRegister(byte(0x0E), adc_setting[5]<<4 | adc_setting[7]);          // F6 right | F8 right
  writeRegister(byte(0x0F), adc_setting[2]<<2);     // F3 right
  writeRegister(byte(0x10), adc_setting[0]);        // F1 right
  writeRegister(byte(0x11), adc_setting[8]<<4);     // CLEAR right
  writeRegister(byte(0x12), byte(0x00));            // Reserved or disabled
  writeRegister(byte(0x13), adc_setting[9]);        // NIR
}
 ...
 */
static float Adafruit_AS7341::toBasicCounts(uint16_t raw, as7341_gain_t gain, uint8_t atime, uint16_t astep) {
  float gain_val = 0;
  switch (gain) {
  ...
  return raw / (gain_val * (atime + 1) * (astep + 1) * 2.78 / 1000);
}
