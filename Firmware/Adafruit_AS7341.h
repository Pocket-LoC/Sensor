...
public:
  ...
  long getTINT();
  static float toBasicCounts(uint16_t raw, as7341_gain_t gain, uint8_t atime, uint16_t astep);
  ...
  bool readResults(uint16_t* readings_buffer);

  uint16_t detectFlickerHz(void);

  void setup_custom(uint8_t adc_config[]);
  void setup_F1F4_Clear_NIR(void);
  void setup_F5F8_Clear_NIR(void);

  void setSMUXCustomChannels(uint8_t adc_config[]);
